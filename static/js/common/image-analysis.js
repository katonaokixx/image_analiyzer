...
// グローバル変数（progressInterval, uploadedImagesDataはimage_upload.htmlで定義済み）
...
function closeModelSelectionModal() {
  ...
  if (modal) {
    ...
    // すべてのバックドロップを削除
    const backdrops = document.querySelectorAll('.overlay-backdrop');
...
/**
 * 解析UI更新
 * @param {string} status - ステータス ('analyzing', 'completed', 'failed')
 * @param {number} progress - 進捗パーセンテージ
 */
  ...
  if (status === 'completed') {
    ...
    if (container) {
      if (container.children.length === 0) {
        ...
        } else if (typeof uploadedImagesData !== 'undefined' && uploadedImagesData.length > 0) {
 ...
 * @returns {Array} アップロード済み画像の配列
 */
function getLocalUploadedImages() {
  console.log('getLocalUploadedImages: 開始');

  // グローバル変数に保存された画像データを使用
  if (typeof uploadedImagesData !== 'undefined' && uploadedImagesData && uploadedImagesData.length > 0) {
 ...
 * @param {Object} statusInfo - ステータス情報
 */
